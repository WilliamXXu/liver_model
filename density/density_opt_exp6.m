function density_opt_exp6()  %two data, two stage model no delay
...
initial=[1 0 0 0];
r=[1 1 1 1 1 1];
%r=[58.495688886188930   1.069419241862175  88.590423971174957   0.004998415576000   0.167122440601395   0.876701707026992];
...
opts = optimoptions(@fmincon,'Display','iter');%,'StepTolerance',1e-6,'OptimalityTolerance',1e-5); %  ,'MaxFunctionEvaluations',500);
problem = createOptimProblem('fmincon','objective',@objectiv,'x0',r,'lb',b.','ub',ub,'options',opts);

ms = MultiStart('StartPointsToRun','bounds-ineqs','UseParallel',1,'Display','iter','MaxTime',1800);
%[r,f] = run(ms,problem,50);
