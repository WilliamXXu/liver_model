function [res]=oxygen_module(consump)

%----------------------  adjustable parameters  ----------------------
unit=2; % miu m, space step
timestep=4.5e-3; %second, time step
error_bound=100;      %mmHg/second
%iteration=100;
%----------------------   parameters ----------------------
...
sinusoid=round(sinusoid_diameter/2/unit);
 %enlarge the entire matrix by 1 unit, for systematic treatment of boundary conditions
global radius
global length
global total
...
%while count<iteration
    ...
    for i=2:(length+1)    % update the oxygen consumption term
        for j=(sinusoid+3):(radius+1)
            n=up(i,j);
            b_current(n)=-timestep*consump(C(n));            
        end
    end
